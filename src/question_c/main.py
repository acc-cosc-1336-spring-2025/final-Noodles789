from question_c import get_most_likely_ancestor_consensus, validate_dna_input

def main():
    print("\nDNA Substring Position Finder")
    
    while True:
        main_dna = validate_dna_input(
            "Enter main DNA sequence (9-16 characters): ",
            min_len=8,
            max_len=17
        )
        
        sub_dna = validate_dna_input(
        ...
        if input("\nCheck another sequence? (y/n): ").lower() != 'y':
            print("Program ended.")
            break
